...
export class Program {
  ...
  processLine(line: string) {
    ...
    if (
      ["cast", "assert"].includes(keyword.toLowerCase()) &&
      Object.keys(Type).includes(param)
    ) {
      this.output +=
        keywords[keyword.toLowerCase() as keyof typeof keywords] +
        Type[param as keyof typeof Type] +
        "\n";
      return;
    }
