...
export function extractBranches(operations: Operation[]) {
  ...
  for (const labelOp of operations.filter(
    (op) => op.instruction === Instruction.Label,
  )) {
    branches.push({
  ...
  for (const operation of operations) {
    switch (operation.instruction) {
      ...
      case Instruction.Call: {
        ...
        currentBranch.nextBranch = newBranch;
        currentBranch.controlFlowBranch = branches.find(
      ...
      case Instruction.Jump: {
        const labelledBranch = branches.find(
          (branch) => branch.label === operation.argument,
        )!;
